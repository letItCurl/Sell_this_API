class V2::ClassifiedsController < V1::ClassifiedsController
    ...
    def index 
        [:page, :per_page, :order].each do |param_sym|
            render json: {error: "missing parameter #{param_sym.to_s}"}, 
            status: :bad_request and return unless params[param_sym]
        end
        render json: {error: 'order must be asc or desc'}, 
        status: :bad_request and return unless params[:order] == 'asc' || params[:order] == 'desc'
